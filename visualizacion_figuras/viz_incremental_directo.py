# -*- coding: utf-8 -*-
"""
Visualización paso a paso del algoritmo Incremental.
...
punto procesado (triángulo inicial, puntos interiores descartados, puntos
exteriores con sus aristas visibles, puntos de tangencia y vértices eliminados).
...
válida bajo la hipótesis de posición general (los puntos de ejemplo cumplen que
los tres primeros no son colineales). La versión robusta de la inicialización
—que localiza el primer triple no colineal— es la del anexo de implementación;
ambas coinciden cuando los tres primeros puntos no están alineados, como aquí.

Ejecución:  python3 viz_incremental.py
...
# ---------------------------------------------------------------------
# Incremental instrumentado: registra cada incorporación de punto
# ---------------------------------------------------------------------

def incremental_pasos(P):
    P = np.asarray(P, float)
    n = len(P)
    if orientacion(P[0], P[1], P[2]) > 0:
        H = [0, 1, 2]
    else:
        H = [0, 2, 1]
    ...
    for k in range(3, n):
        ...
        removidos = [v for v in H_antes if v not in H]
...
# ---------------------------------------------------------------------
# Dibujo paso a paso, con número de orden en cada punto
# ---------------------------------------------------------------------

def dibujar_incremental_pasos(P, pasos, hull, ncols=3, archivo="incremental_directo.pdf"):
    plt.close('all')
    P = np.asarray(P, float)
    COL_PROC, COL_HULL = "#dc2626", "#1f2937"
    COL_VIS, COL_TAN, COL_NUEVA = "#dc2626", "#2563eb", "#16a34a"
    COL_DESC, COL_ELIM, COL_NUM = "#9ca3af", "#d97706", "#64748b"
    S_PT = 26
    orden_vertices = {idx: k for k, idx in enumerate(hull)}

    xmin, xmax = P[:, 0].min(), P[:, 0].max()
    ymin, ymax = P[:, 1].min(), P[:, 1].max()
    mx, my = 0.16 * (xmax - xmin), 0.16 * (ymax - ymin)
    ...
    def numeros_orden(ax):
        for idx in range(len(P)):
            ax.annotate(str(idx), P[idx], textcoords="offset points", xytext=(4, 5),
                        fontsize=7, color=COL_NUM, annotation_clip=False, zorder=10)
    ...
    for k_idx, paso in enumerate(pasos):
        ...
        if paso["tipo"] == "init":
            ...
            if paso["interior"]:
                ax.scatter([P[k, 0]], [P[k, 1]], color=COL_DESC, marker="x",
                           s=S_PT+30, zorder=8, label="descartado (interior)")
                ax.legend(fontsize=7, loc="upper left", framealpha=0.9)
                ax.set_title(f"Paso {k_idx+1}: $p_{{{k}}}$ interior, se descarta", fontsize=9)
            else:
                ...
                etq(ax, k, f"$p_{{{k}}}$", COL_PROC, dx=8, dy=6, bold=True)
                if paso["tangencias"] or paso["removidos"]:
                    ax.legend(fontsize=7, loc="upper left", framealpha=0.9)
                titulo_extra = " (elimina vértices)" if paso["removidos"] else ""
                ax.set_title(f"Paso {k_idx+1}: $p_{{{k}}}$ exterior{titulo_extra}", fontsize=9)
    ...
    fig.suptitle("Ejecución del algoritmo Incremental (orden de entrada)", fontsize=14)
...
if __name__ == "__main__":
    P = np.array([
        [0, 3], [10, 1], [8, 11],   # 0,1,2 -> triángulo inicial
        [5, 5],                     # 3 interior
        [12, 6],                    # 4 exterior derecha
        [3, 6],                     # 5 interior
        [4, -2],                    # 6 exterior abajo
        [1, 10],                    # 7 exterior arriba-izq
        [-3, 7],                    # 8 exterior izquierda
        [6, 3],                     # 9 interior
        [7, 7],                     # 10 interior
        [4, 2],                     # 11 interior
    ], dtype=float)
    ...
    dibujar_incremental_pasos(P, pasos, hull, ncols=3, archivo="incremental_directo.pdf")
    print("Figura guardada en incremental_directo.pdf")
